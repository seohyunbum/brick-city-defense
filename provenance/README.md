...
공개 배포 폴더는 `_site/`다. `npm run build`는 최종 조립 뒤 HTML 비가시 서명, 파일 전체 매니페스트, 고정 공개키 대조를 실행하며 실패하면 빌드도 실패한다. 서버·클라이언트 중간 빌드는 최종 배포물과 구분한다.
...
공개 검증 원장: https://github.com/seohyunbum/brick-city-defense/tree/hbsy-provenance
