...
`npm run smoke:pilot`은 같은 페이지의 86호기 조종 모드를 검사한다. 조종석 진입과 눈높이,
웨이브 등장, 실제 전진, 기관총·칼로 기계가 쓰러지는지, 장갑이 0 이 되어도 게임오버 없이 수리 후
이어지는지, 기계 10대를 동시에 세운 최악의 프레임 예산, 단편으로 돌아왔을 때 배우·하늘·칼이
원래대로 돌아오는지를 본다(최근 실측 draw calls 143, triangles 40,536).
